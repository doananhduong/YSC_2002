...
def processing_data():
  if df[df['is_processed'] == False].empty:
    ...
    pandas_gbq.to_gbq(df_process, 'feature_store.feature_data', project_id="modular-seeker-345718", if_exists = 'append')
    query_result = bigquery_client.query(
      """
      SELECT
        *
      FROM
        `modular-seeker-345718.feature_store.feature_data`
      """
    )
